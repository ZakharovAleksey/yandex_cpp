...
void TestAddingDocumentsToTheServer() {
...

    {
        ...
        ASSERT_EQUAL_HINT(server.GetDocumentCount(), 1, "Should add document which consists only from spaces"s);
    }
}
...
void TestServerMatchWordsForTheDocument() {
    ...
    ASSERT_EQUAL_HINT(matching_words[0], "cat"s, "Server match the exact word from the query");
    ASSERT_EQUAL_HINT(matching_words[1], "puppy"s, "Server match the exact word from the query");
...
void TestDocumentRelevanceCalculation() {
    ...
    auto calculate_relevance = [&words_rating_data](int document_id) {
        ...
        for (const auto& [word, index_to_frequency] : words_rating_data) {
            ...
            relevance += log(kDocumentsCount * 1. / documents_with_word_count) * index_to_frequency.at(document_id);
        }
        return relevance;
    };
    ...
    for (const auto& [document_id, actual_relevance] : actualRelevance) {
        ...
        ASSERT_HINT(doubles_equal(expected_relevance, actual_relevance), "Documents has expected relevance values"s);
    }
}
...
void TestFindDocumentsWithCustomDocumentStatusFilterFunction() {
    ...
    for (int document_id = general_document_id; document_id < general_document_id + kDocumentsCount; ++document_id) {
...
void TestDocumentsCount() {
    ...
    ASSERT_EQUAL_HINT(server.GetDocumentCount(), 0, "Initially server does not have any documents"s);
    ...
    ASSERT_EQUAL_HINT(server.GetDocumentCount(), kAddedDocumentsCount, "Server stored all added document"s);
}
