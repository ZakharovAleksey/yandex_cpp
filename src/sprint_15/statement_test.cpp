...
#define ASSERT_OBJECT_VALUE_EQUAL(obj, expected)                                                       \
    {                                                                                                  \
        ...
        __assert_equal_private_os << #obj << "'s value_ "sv                                            \
                                  << " != "sv << #expected << "s, "sv << FILE_NAME << ':' << __LINE__; \
...
void TestFieldAssignment() {
    ...
    FieldAssignment assign_yz(VariableValue{vector<string>{"self"s, "y"s}}, "z"s,
...
void TestBadAddition() {
    ...
    ASSERT_THROWS(Add(make_unique<NumericConst>(42), make_unique<StringConst>("4"s)).Execute(empty, context),
                  std::runtime_error);
    ASSERT_THROWS(Add(make_unique<StringConst>("4"s), make_unique<NumericConst>(42)).Execute(empty, context),
                  std::runtime_error);
...
void TestSuccessfulClassInstanceAdd() {
    ...
    methods.push_back({"__add__"s,
                       {"value_"s},
                       make_unique<Add>(make_unique<StringConst>("hello, "s), make_unique<VariableValue>("value_"s))});
...
void TestFields() {
    ...
    methods.push_back({"__init__"s,
                       {},
                       {make_unique<FieldAssignment>(VariableValue{"self"s}, "value"s, make_unique<NumericConst>(0))}});
...
void TestBaseClass() {
    ...
    methods.push_back(
        {"SetValue"s,
         {"x"s},
         make_unique<FieldAssignment>(VariableValue{"self"s}, "value"s, make_unique<ast::VariableValue>("x"s))});
...
void TestInheritance() {
    ...
    methods.push_back(
        {"SetValue"s,
         {"x"s},
         make_unique<FieldAssignment>(VariableValue{"self"s}, "value"s, make_unique<VariableValue>("x"s))});
...
void TestOr() {
    auto test_or = [](bool lhs, bool rhs) {
        ...
        runtime::DummyContext context;
        ASSERT_EQUAL(
            runtime::Equal(or_statement.Execute(closure, context), ObjectHolder::Own(runtime::Bool(true)), context),
            lhs || rhs);
    };
...
void TestAnd() {
    auto test_and = [](bool lhs, bool rhs) {
        ...
        runtime::DummyContext context;
        ASSERT_EQUAL(
            runtime::Equal(and_statement.Execute(closure, context), ObjectHolder::Own(runtime::Bool(true)), context),
            lhs && rhs);
    };
...
void TestNot() {
    auto test_not = [](bool arg) {
        ...
        runtime::DummyContext context;
        ASSERT_EQUAL(
            runtime::Equal(not_statement.Execute(closure, context), ObjectHolder::Own(runtime::Bool(true)), context),
            !arg);
    };
