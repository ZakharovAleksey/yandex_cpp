...
bool IsPalindrome(const std::string &text) {
    ...
    text_copy.erase(std::remove_if(text_copy.begin(), text_copy.end(), [](char s) { return s == ' '; }),
                    text_copy.end());
...
std::ostream &operator<<(std::ostream &os, const BusesForStopResponse &response) {
    ...
    switch (response.status) {
        ...
        case ResponseStatus::Ok:
            std::copy(response.stops.begin(), response.stops.end(), std::ostream_iterator<std::string>(os, " "));
            break;
    }
...
void BusManager::AddBus(const std::string &bus, const std::vector<std::string> &stops) {
    ...
    std::for_each(stops.begin(), stops.end(),
                  [&](const std::string &stop) { stops_to_buses_[stop].emplace_back(bus); });
}
...
StopsForBusResponse BusManager::GetStopsForBus(const std::string &bus) const {
    ...
    for (const auto &stop : buses_to_stops_.at(bus)) {
        ...
        std::copy_if(stops_to_buses_.at(stop).begin(), stops_to_buses_.at(stop).end(), std::back_inserter(buses),
                     [bus](const std::string &other_bus) { return other_bus != bus; });
